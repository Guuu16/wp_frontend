<template>
    <div>
        <Card>
            ...
            <template #bottomLeft>
                <span class="mr-4">LOOP</span>
                <el-input-number v-model="loopValue" :min="1" :max="10000" />
            </template>
...
const getData = async (release = null) => {
    ...
    await store.dispatch('getJobs', 'performance').then(res => {
...
const changeTestItems = (val) => {
    testItemsValue.value = val
}
...
const startStatus = computed(() => {
    if (systemsValue.value.length != 0 && testItemsValue.value.length != 0) {
        if (releaseValue.value && modeValue.value && loopValue.value && receiverValue.value) {
