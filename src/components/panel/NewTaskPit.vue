            ...
            <template #body>
                <el-row :gutter="20" v-loading="loading" :element-loading-text="loadingValue">
                    ...
                    <el-col :span="8" :offset="0">
                        <NewTaskItem ref="stressItemsRef" :loading="itemsLoading" title="STRESS TEST" :items="stressItems"
                            @change="changeStressItems">
                            <template #headerRight>
                                <span class="mr-4 text-sm">LOOP</span>
                                <el-input-number size="small" v-model="loopValue" :min="1" :max="10000" />
                            </template>
                        </NewTaskItem>
                    </el-col>
                </el-row>
...
const getData = async (release = null) => {
    ...
    await store.dispatch('getJobs', 'mrt').then(res => {
...
// stress items
const stressItemsRef = ref()
const loopValue = ref(1)
const stressItemsValue = ref([])
const stressItems = computed(() => {
    if (releaseValue.value) {
        return store.state.releaseOptions.filter(res => res.id == releaseValue.value)[0].testItems.stress
    } else return []
})

const changeStressItems = (val) => {
    stressItemsValue.value = val
}
...
const isUploadXccBuild = computed(() => {
    if (systemsValue.value.length != 0) {
        let res = xccItemsValue.value.includes('flashXCC') || stressItemsValue.value.includes('flashXCCStress')
        return res
    } return false
})
const isUploadUefiBuild = computed(() => {
    if (systemsValue.value.length != 0) {
        let res = uefiItemsValue.value.includes('flashUEFI') || stressItemsValue.value.includes('flashUEFIStress')
        return res
    } return false
})
...
const isStart = computed(() => {
    if (systemsValue.value.length != 0 && [...xccItemsValue.value, ...uefiItemsValue.value, ...stressItemsValue.value].length != 0) {
